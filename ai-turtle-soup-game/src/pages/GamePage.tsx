...
import type { Story, QuestionAnswer, AnswerType } from '../types/game';
...
const GamePage: React.FC = () => {
  ...
  const [aiService] = useState(() => new AIService(false));
  ...
  const handleSubmitQuestion = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 显示错误提示
      toast.error('获取回答失败', '请检查网络连接或稍后重试');
    } finally {
  ...
  const handleUseHint = () => {
    ...
    if (hintIndex < story.hints.length) {
      ...
      toast.info(`提示 ${hintIndex + 1}`, hint, 8000);
      setGameSession(prev => ({ ...prev, hintsUsed: prev.hintsUsed + 1 }));
    } else {
      toast.warning('提示用尽', '你已经使用了所有可用提示');
    }
  };
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-lg md:text-xl font-bold text-gray-900">{story.title}</h1>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 lg:sticky lg:top-6">
              <h2 className="text-lg md:text-xl font-bold text-gray-900 mb-4">汤面故事</h2>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-md border border-gray-200 flex flex-col h-[calc(100vh-160px)] md:h-[calc(100vh-200px)]">
              {/* 消息区域 */}
              <div className="flex-1 overflow-y-auto p-6">
                {questions.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {questions.map((q) => (
                      <div key={q.id} className={cn('space-y-2', 'fade-in')}>
                        {/* 玩家问题 */}
