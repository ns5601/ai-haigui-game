...
export class RuleEngine {
   ...
   * 更新关键词模式（可用于动态学习）
   */
  updatePatterns(patterns: Partial<typeof this.keywordPatterns>): void {
    this.keywordPatterns = { ...this.keywordPatterns, ...patterns };
  }

  /**
   * 添加自定义关键词
   */
  addKeyword(type: keyof typeof this.keywordPatterns, keyword: string): void {
    if (!this.keywordPatterns[type].includes(keyword)) {
      this.keywordPatterns[type].push(keyword);
    }
  }
